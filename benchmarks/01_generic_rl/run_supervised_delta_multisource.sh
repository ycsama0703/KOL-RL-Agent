...
OUTPUT_ROOT=${OUTPUT_ROOT:-outputs/benchmarks/generic_rl/supervised_delta}
...
for task in "${TASKS[@]}"; do
  ...
  echo "Launch Supervised-Delta benchmark for $rel -> $log_file"
  nohup "$PYTHON" train.py \
    ...
    --bc-fit-behavior \
    --bc-anchor-lambda 0.03 \
    --iql-steps 0 \
    --hard-intent-constraints \
    --fidelity-lambda 0.0 \
    --actor-align-lambda 0.0 \
    --entry-penalty-lambda 0.02 \
    --reversal-penalty-lambda 0.05 \
    --no-progress-bar \
    >"$log_file" 2>&1 &
