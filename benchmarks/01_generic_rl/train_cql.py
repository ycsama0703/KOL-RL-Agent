"""Train CQL benchmark on existing replay buffers.
...
class CQLConfig:
    ...
    output_dir: str = "outputs/benchmarks/generic_rl/cql"
    ...
    # Actor update follows TD3+BC-style adaptive Q-term weighting.
    actor_alpha: float = 2.5
...
def parse_args() -> CQLConfig:
    p = argparse.ArgumentParser(description="Train CQL benchmark.")
    ...
    p.add_argument("--actor-alpha", type=float, default=2.5)

    p.add_argument("--cql-alpha", type=float, default=1.0)
    p.add_argument("--cql-temp", type=float, default=1.0)
    p.add_argument("--cql-n-actions", type=int, default=10)
    p.add_argument("--action-min", type=float, default=-1.0)
    p.add_argument("--action-max", type=float, default=1.0)
...
def compute_cql_penalty(
    ...
    *,
    ...
    batch_size = ext_state.shape[0]
    action_dim = behavior_action.shape[-1]

    random_actions = torch.empty(
        batch_size, n_actions, action_dim, device=ext_state.device
    ).uniform_(action_min, action_max)
    state_rep = ext_state.unsqueeze(1).expand(-1, n_actions, -1).reshape(batch_size * n_actions, -1)
    q_rand = critic(state_rep, random_actions.reshape(batch_size * n_actions, action_dim)).reshape(batch_size, n_actions)

    q_pi = critic(ext_state, policy_action).reshape(batch_size, 1)
    q_data = critic(ext_state, behavior_action).reshape(batch_size, 1)

    q_cat = torch.cat([q_rand, q_pi], dim=1)
    cql = (torch.logsumexp(q_cat / temp, dim=1, keepdim=True) * temp - q_data).mean()
    return cql
...
def main() -> None:
    ...
    LOGGER.info("Starting CQL benchmark run %s", run_name)
    ...
    csv_writer = None
    if cfg.write_csv:
        csv_path = log_dir / "cql_metrics.csv"
        ...
        csv_fp.flush()
        LOGGER.info("CQL metrics CSV: %s", csv_path)
    ...
    if cfg.progress_bar:
        steps_iter = tqdm(steps_iter, desc="CQL Training")
    ...
    for step in steps_iter:
        ...
        with torch.no_grad():
            policy_action_detached = build_action(actor, state, baseline_action, cfg.clamp_delta)
        cql1 = compute_cql_penalty(
            critic1,
            ext_state,
            behavior_action,
            policy_action_detached,
            n_actions=cfg.cql_n_actions,
            action_min=cfg.action_min,
            action_max=cfg.action_max,
            temp=cfg.cql_temp,
        )
        cql2 = compute_cql_penalty(
            critic2,
            ext_state,
            behavior_action,
            policy_action_detached,
        ...
        cql_loss = cql1 + cql2
        critic_loss = bellman_loss + cfg.cql_alpha * cql_loss
        ...
        lambda_value = 0.0
        policy_abs_mean = float(policy_action_detached.abs().mean().item())
        ...
        if step % cfg.policy_freq == 0:
            ...
            lambda_coef = cfg.actor_alpha / q_mean_abs
        ...
        rolling["bellman_loss"] += float(bellman_loss.item())
        rolling["cql_loss"] += float(cql_loss.item())
        ...
        if step % max(cfg.log_interval, 1) == 0 or step == cfg.steps:
            ...
            LOGGER.info(
                "CQL step %d/%d | critic=%.6f bellman=%.6f cql=%.6f actor=%.6f bc=%.6f q_term=%.6f lambda=%.6f q_data=%.6f target_q=%.6f reward=%.6f done=%.4f | |pi|=%.6f |beh|=%.6f",
                step,
                cfg.steps,
                avg["critic_loss"],
                avg["bellman_loss"],
