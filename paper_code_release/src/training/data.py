...
class ReplayDataset(Dataset):
    def __init__(self, buffer_path: str | Path) -> None:
        ...
        self.next_baseline_actions = data.get("next_baseline_action", self.baseline_actions).float()
        # Prefer portfolio-level rewards when present; otherwise fall back to per-ticker reward_1d.
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:  # type: ignore[override]
        return {
            ...
            "action": self.actions[idx],  # Behavior action stored in the replay buffer.
            "baseline_action": self.baseline_actions[idx],
            "next_baseline_action": self.next_baseline_actions[idx],
