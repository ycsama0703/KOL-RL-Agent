...
def build_buffer(
    ...
    *,
    ...
    buffer = {
        ...
        # Keep per-ticker reward_1d for replay/evaluation. Store the portfolio-level
        # reward separately for training.
        ...
        # Behavior action used to train actor/critic.
        "actions": torch.from_numpy(behavior).unsqueeze(-1),
        # Baseline action used as the intent anchor.
        "baseline_actions": torch.from_numpy(baseline).unsqueeze(-1),
        # Next-step baseline action used by residual-aware value estimation.
...
def process_file(
    ...
    *,
    ...
    # Portfolio-level reward based on behavior weights and per-ticker returns.
