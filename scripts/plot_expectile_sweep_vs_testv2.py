#!/usr/bin/env python
"""Plot daily equity for expectile sweep (0.5/0.6) vs test_v2 per KOL."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare expectile sweep vs test_v2 (daily).")
    parser.add_argument("--sweep-root", default="outputs/sweep_expectile/test")
    parser.add_argument("--testv2-root", default="outputs/test_v2")
    parser.add_argument("--output-root", default="outputs/sweep_expectile/compare_testv2")
    parser.add_argument("--expectiles", default="0.5 0.6")
...
def main() -> None:
    ...
    exps = [e.strip() for e in args.expectiles.split() if e.strip()]
    if not exps:
        raise SystemExit("No expectiles provided.")

    # Use test_v2 KOL list as base, fall back to sweep if empty.
    kols = list_kols(testv2_root)
    if not kols and sweep_root.exists():
        # Look under first sweep directory for KOLs.
        sample_dir = sweep_root / f"exp{exps[0]}_beta{args.beta}"
    ...
    for kol in sorted(kols):
        ...
        # sweep expectiles
        for exp in exps:
            sweep_dir = sweep_root / f"exp{exp}_beta{args.beta}"
            if not sweep_dir.exists():
                continue
            run = latest_run_dir(sweep_dir, kol)
            if not run:
                continue
            daily_path = run / "daily" / "equity_daily.csv"
            df = read_equity_daily(daily_path)
            if df is not None:
                series[f"exp{exp}"] = df
        ...
        csv_path = out_dir / "compare_expectile_daily.csv"
        ...
        ax.set_title(f"{kol} Expectile Sweep vs {args.label_testv2}")
        ax.grid(True, alpha=0.3)
        ax.legend()
        fig.autofmt_xdate()

        fig_path = out_dir / "compare_expectile_daily.png"
